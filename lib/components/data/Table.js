...
export function Table({ ariaLabel, columns, rows, getRowKey, onRowClick, compact, initialSortKey, initialSortDescending, hideFilters, emptyLabel = "No rows match the current filters.", searchValue, searchLabel = "Search", maxHeight = 620, fill, extraFilters, }) {
    ...
    const fluentColumns = columns.map((column) => {
        ...
        return ({
            ...
            maxWidth: (_d = (_c = (_b = widths[column.key]) !== null && _b !== void 0 ? _b : column.maxWidth) !== null && _c !== void 0 ? _c : column.minWidth) !== null && _d !== void 0 ? _d : 100,
            isResizable: true,
            isMultiline: false,
            isPadded: true,
    ...
    return (React.createElement("div", { style: { display: "grid", gap: Theme.tokens.space.sm, minWidth: 0 } },
        ...
        React.createElement("div", { style: {
            ...
            React.createElement(DetailsList, { styles: {
                ...
                }, ariaLabel: ariaLabel, items: page, columns: fluentColumns, getKey: (item) => getRowKey(item), selectionMode: SelectionMode.none, layoutMode: DetailsListLayoutMode.fixedColumns, constrainMode: ConstrainMode.horizontalConstrained, compact: compact, onShouldVirtualize: () => false, onColumnResize: (column, newWidth) => {
                    if (!column || newWidth === undefined)
                        return;
                    setWidths((current) => (Object.assign(Object.assign({}, current), { [column.key]: newWidth })));
                }, onRenderRow: (props) => {
                    if (!props)
                        return null;
                    if (!onRowClick)
