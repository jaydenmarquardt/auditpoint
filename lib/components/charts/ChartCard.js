...
const ChartCardView = ({ title, info, points, charts: requestedCharts = ["hbar", "bar", "donut"], defaultChart, valueFormatter, centreLabel, emptyLabel, footer, previewCount = 10, height = 410, span = 1, }) => {
    ...
    const legend = (data) => (React.createElement("ul", { style: {
        ...
        return (React.createElement("li", { key: point.label },
            React.createElement("button", { type: "button", onClick: () => toggle(point.label), "aria-pressed": !off, title: `${point.label}: ${valueFormatter ? valueFormatter(point.value) : point.value.toLocaleString()}`, style: {
                    display: "inline-flex",
                    alignItems: "center",
                    gap: 6,
                    maxWidth: 260,
