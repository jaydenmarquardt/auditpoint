...
export const BarChart = ({ ariaLabel, points, title, height = 220, horizontal, valueFormatter = (value) => value.toLocaleString(), colour, emptyLabel, }) => {
    ...
    return (React.createElement(ChartFrame, { title: title, ariaLabel: ariaLabel },
        React.createElement("div", { ref: ref, style: { width: "100%", minWidth: 0, height } },
            React.createElement(VerticalBarChart, { data: bars, width: width, height: height, wrapXAxisLables: true, showXAxisLablesTooltip: true, hideLegend: true, barWidth: "auto", yAxisTickCount: 4, culture: undefined }))));
};
/** Enough to recognise a page, short enough that ten of them fit across a card. */
function shorten(label) {
    var _a;
    const value = `${label !== null && label !== void 0 ? label : ""}`;
    if (value.length <= 18)
        return value;
    const tail = (_a = value.split("/").filter(Boolean).pop()) !== null && _a !== void 0 ? _a : value;
