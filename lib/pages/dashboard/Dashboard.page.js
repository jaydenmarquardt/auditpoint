...
const Dashboard = () => {
    ...
    return (React.createElement(React.Fragment, null,
        ...
        React.createElement("h2", { style: { fontSize: Tokens.font.lg, margin: `0 0 ${Tokens.space.md}` } }, DashboardContent.toolsTitle),
        React.createElement("div", { style: {
                display: "grid",
                gridTemplateColumns: "repeat(auto-fill, minmax(min(260px, 100%), 1fr))",
                gap: Tokens.space.md,
            } }, ROUTES.filter((route) => route.key !== "dashboard" && !route.hidden && isModuleEnabled(route.key, disabledModules)).map((route) => {
