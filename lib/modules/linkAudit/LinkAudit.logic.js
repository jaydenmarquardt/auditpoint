...
export function untestedReason(link, checked) {
    if (link.broken !== "unsure")
        return "";
    if (link.linkType === "external")
        return LinkAuditContent.reasons.external;
    if (link.linkType === "share")
        return LinkAuditContent.reasons.share;
    if (!checked)
        return LinkAuditContent.reasons.notChecked;
    if (link.status === 0)
        return LinkAuditContent.reasons.noAnswer;
    return LinkAuditContent.reasons.unknown;
}
export function untestedUsages(links) {
