...
export const LinkAuditStats = ({ view, config, previous }) => {
    const before = previous ? statSections(previous, config) : undefined;
    const sections = statSections(view, config).map((section, index) => {
        var _a;
        return (Object.assign(Object.assign({}, section), { tiles: compareTiles(section.tiles, (_a = before === null || before === void 0 ? void 0 : before[index]) === null || _a === void 0 ? void 0 : _a.tiles) }));
    });
    return (React.createElement("div", { style: { display: "grid", gap: Theme.tokens.space.lg, width: "100%", minWidth: 0 } }, sections.map((section) => (React.createElement(StatGrid, { key: section.key, title: section.title, tiles: section.tiles, minWidth: 180 })))));
};
