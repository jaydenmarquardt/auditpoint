...
import { flagsFor, sourceLabel, statusLabel, untestedReason } from "./LinkAudit.logic";
...
export const usageColumns = [
    ...
    },
    {
        ...
        minWidth: 110,
        render: (usage) => usage.reference.url ? (React.createElement(Button, { label: LinkAuditContent.columns.open, variant: "subtle", iconName: "OpenInNewWindow", newTab: true, href: absoluteFromServerRelative(usage.reference.url, usage.reference.siteUrl || window.location.href) })) : (React.createElement("span", null, "-")),
    },
];
/** The untested view adds the one column that view exists to answer. */
export function untestedColumns(checked) {
    return [
        ...usageColumns.filter((column) => column.key !== "open"),
        {
            ...
            render: (usage) => React.createElement("span", null, untestedReason(usage.link, checked)),
        },
        ...usageColumns.filter((column) => column.key === "open"),
    ];
}
