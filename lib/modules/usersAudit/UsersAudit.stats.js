...
export const UsersAuditStats = ({ view, config, }) => {
    ...
    return (React.createElement("div", { style: {
        ...
        React.createElement(StatTile, { iconName: "Group", label: UsersAuditContent.stats.groups, value: config.readGroups ? formatNumber(totals.groups) : "-", info: UsersAuditContent.tileInfo.groups }),
        React.createElement(StatTile, { iconName: "Calculator", label: UsersAuditContent.stats.average, value: config.readGroups ? String(totals.averageGroupSize) : "-", info: UsersAuditContent.tileInfo.average }),
        React.createElement(StatTile, { iconName: "UserOptional", label: UsersAuditContent.stats.ungrouped, value: config.readGroups ? formatNumber(totals.usersWithoutGroup) : "-", tone: "warning", info: UsersAuditContent.tileInfo.ungrouped }),
        React.createElement(StatTile, { iconName: "ContactCard", label: UsersAuditContent.stats.profiles, value: config.readProfiles ? formatNumber(totals.profilesRead) : "-", hint: config.readProfiles ? `${formatNumber(totals.withDepartment)} with a department` : undefined, info: UsersAuditContent.tileInfo.profiles })));
};
