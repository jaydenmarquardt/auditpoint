...
export const ReportRunPanel = ({ title, controller, runLabel, extraControls, menuItems = [], runDisabled, configPanel, configOpen, onConfigOpenChange, onBack, backLabel = "All runs", definition, }) => {
    ...
    return (React.createElement("div", { style: { display: "grid", gap: Theme.tokens.space.md } },
        ...
        controller.loading && React.createElement(ReportSkeleton, { label: `Opening ${title}` }),
        ...
        envelope && (React.createElement(ProgressGroup, { label: activeLabel(title, steps), status: mapRunStatus(envelope.status), ratio: overall, steps: steps, description: `Updated ${formatDateTime(envelope.updatedIso)} · ${envelope.issues.length} issues · ${throttle.inFlight} in flight` }))));
};
...
function activeLabel(title, steps) {
