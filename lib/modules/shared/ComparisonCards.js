...
export const ComparisonCards = ({ sections }) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(ChartCard, { title: "Before and after", info: "The measures that moved most, each shown as it was and as it is now.", defaultChart: "bar", charts: ["bar", "hbar"], span: 2, selectable: false, valueFormatter: (value) => value.toLocaleString(), points: movers.flatMap((tile) => {
                var _a, _b;
                return [
                    { label: `${tile.label} — was`, value: (_a = tile.previousValue) !== null && _a !== void 0 ? _a : 0, colour: before },
                    { label: `${tile.label} — now`, value: (_b = tile.currentValue) !== null && _b !== void 0 ? _b : 0, colour: after },
                ];
            }) }),
        React.createElement(ChartCard, { title: "Biggest changes", info: "Measures that moved the most between the two runs, by count.", defaultChart: "hbar", charts: ["hbar", "bar"], span: 2, selectable: false, valueFormatter: (value) => value.toLocaleString(), points: movers.map((tile) => ({
                label: tile.label,
                value: change(tile),
                colour: tone(change(tile)),
            })) }),
        React.createElement(ChartCard, { title: "Change by percentage", info: "The same comparison as a share of the earlier run, so small measures are not drowned out.", defaultChart: "hbar", charts: ["hbar", "bar"], span: 2, selectable: false, valueFormatter: (value) => `${value}%`, points: byPercent.map((tile) => ({
                label: tile.label,
                value: percent(tile),
