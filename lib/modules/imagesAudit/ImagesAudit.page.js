...
import { statTiles } from "./ImagesAudit.stats";
...
const ImagesAuditPage = () => {
    ...
    return (React.createElement(React.Fragment, null,
        ...
        !controller.envelope && !controller.running && (React.createElement(ReportHistory, { kind: imagesAuditReport.kind, title: ImagesAuditContent.historyTitle, newLabel: ImagesAuditContent.run, busy: controller.running || controller.loading, onNew: () => setConfigOpen(true), onOpen: (url) => void controller.open(url), onResume: (url) => void controller.resumeSaved(url), onImport: (file) => void controller.importJson(file), error: controller.error, onDismissError: controller.clearError })),
        React.createElement(ReportRunPanel, { title: imagesAuditReport.title, controller: controller, definition: imagesAuditReport, onBack: controller.envelope ? controller.clear : undefined, backLabel: ImagesAuditContent.backToRuns, configOpen: configOpen, onConfigOpenChange: setConfigOpen, configPanel: React.createElement(ReportConfigPanel, { bare: true, title: ImagesAuditContent.configTitle, definition: imagesAuditReport, config: controller.config, onChange: controller.setConfig }), menuItems: hasData
                ? [
            ...
            React.createElement(Tabs, { ariaLabel: ImagesAuditContent.title, selectedKey: tab, onChange: setTab, items: [
                    {
                        ...
                        content: React.createElement(OverviewTab, { view: view, hasData: hasData, onRun: () => setConfigOpen(true), previousTiles: previousTiles, comparisonCards: previousTiles ? (React.createElement(ComparisonCards, { sections: [{ title: "", tiles: compareTiles(statTiles(view), previousTiles) }] })) : undefined, comparison: hasData ? (React.createElement(ComparisonBar, { kind: imagesAuditReport.kind, currentId: (_f = controller.envelope) === null || _f === void 0 ? void 0 : _f.id, onChange: (next) => setPreviousData(next) })) : undefined }),
                    },
                    {
                        key: "files",
