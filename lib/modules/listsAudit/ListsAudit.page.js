...
import { statTiles } from "./ListsAudit.stats";
            ...
            React.createElement(Tabs, { ariaLabel: ListsAuditContent.title, selectedKey: tab, onChange: setTab, items: [
                    {
                        ...
                        content: (React.createElement(OverviewTab, { view: view, config: config, hasData: hasData, onRun: () => setConfigOpen(true), previousTiles: previousTiles, comparisonCards: previousTiles ? (React.createElement(ComparisonCards, { sections: [{ title: "", tiles: compareTiles(statTiles({ view, config }), previousTiles) }] })) : undefined, comparison: hasData ? (React.createElement(ComparisonBar, { kind: listsAuditReport.kind, currentId: (_f = controller.envelope) === null || _f === void 0 ? void 0 : _f.id, onChange: (next) => setPreviousData(next) })) : undefined })),
                    },
                    {
                        key: "lists",
