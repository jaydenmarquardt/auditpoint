...
import { LinkScanner, originOf, placement } from "./Links.api";
...
export function ItemAttachments(webUrl) {
    const site = webUrl !== null && webUrl !== void 0 ? webUrl : "";
    return {
        /**
        ...
        inList(list, max) {
            return __awaiter(this, void 0, void 0, function* () {
                const items = (yield throttled(() => getSp(webUrl)
                    .web.lists.getById(list.id)
                    .items.select("Id", "Title", "FileRef", "Modified")
                    .filter("Attachments eq 1")
                    .top(max)(), { label: "Attachments.items" }));
                const files = yield Promise.all(items.map((item) => __awaiter(this, void 0, void 0, function* () {
                    const attachments = (yield throttled(() => getSp(webUrl).web.lists.getById(list.id).items.getById(item.Id).attachmentFiles(), { label: "Attachments.files" }));
                    return attachments.map((attachment) => {
                        ...
                        return ({
                            ...
                            sizeBytes: 0,
                            modified: String((_a = item.Modified) !== null && _a !== void 0 ? _a : ""),
                        });
                    });
                })));
                return files.flat();
