export const SettingsContent = {
  title: "Settings",
  description:
    "Settings are edited here and stored as JSON on the web part. The property pane only exposes the raw JSON.",
  readOnlyNotice: "The page is in read mode, so settings cannot be saved. Switch to edit mode to change them.",
  enterEditMode: "Enter edit mode",
  unsavedNotice: "Settings applied. Save the page to keep them.",
  save: "Apply settings",
  reset: "Reset changes",
  branding: "Branding",
  reports: "Report storage",
  sites: "Sites in scope",
  performance: "Performance",
  fields: {
    appName: "App name",
    appTagline: "Tagline",
    reportLibrary: "Library",
    reportFolder: "Folder",
    path: "Full path",
    concurrency: "Parallel requests",
    defaultRoute: "Landing page",
    captureReportLogs: "Capture report logs",
    captureReportLogsHint: "Keeps a run log on each report for debugging. Turn off to keep reports small.",
    additionalSites: "Additional site URLs",
    additionalSitesHint:
      "One absolute URL per line. The host site is always included. A run audits one of these at a time, picked when the run starts.",
  },
  folder: {
    title: "Report folder",
    ensure: "Create or repair folder",
    check: "Re-check",
    exists: "Folder ready",
    missing: "Folder missing",
    noEdit: "You can view this folder but not write to it",
    noView: "No access to this location",
    checking: "Checking folder…",
    permissions: "Permissions",
    view: "View",
    edit: "Edit",
  },
  throttle: {
    title: "Live request state",
    inFlight: "In flight",
    queued: "Queued",
    completed: "Completed",
    retries: "Retries",
    failed: "Failed",
    pause: "Pause requests",
    resume: "Resume requests",
  },
  configWarning: "Modules stay disabled until these settings are complete:",
  mapping: {
    title: "Column mapping",
    description:
      "Which columns this site uses for the things the audits look for. Leave one empty and the audits skip that measure rather than guessing at a name.",
    organisationalUnit: "Organisational unit column",
    organisationalUnitHint: "Internal name of the column holding the owning area, such as OrgUnit.",
    organisationalUnitList: "Organisational unit list",
    organisationalUnitListHint: "Title of the list the column looks up, when it is a lookup.",
    expiryDate: "Expiry date column",
    reviewDate: "Review date column",
    publishDate: "Publish date column",
    htmlFields: "Rich text columns",
    htmlFieldsHint:
      "Comma separated internal names read for content, image and link audits, on top of anything detected automatically.",
  },
  legacy: {
    title: "Retired hosts",
    description:
      "Hosts that have been switched off, one per line. Every link pointing at one is reported as dead without being requested.",
    label: "Retired hosts and urls",
  },
  modules: {
    title: "Modules",
    description:
      "Switch off anything this site does not need. A module switched off here is hidden from the sidebar, the dashboard and routing; its saved reports stay readable.",
    hostLimited: "The page hosting this app offers only the modules listed here.",
    enableAll: "Enable all",
    disableAll: "Disable all",
    enabled: "On",
    disabled: "Off",
  },
  host: "Host site",
  additional: "Additional site",
} as const;
