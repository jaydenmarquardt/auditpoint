...
export function listColumns(target: number): TableColumn<ListIndexCoverage>[] {
  return [
    {
      key: "title",
    ...
    },
    {
      ...
      render: (list) => <Badge label={list.templateName} tone="neutral" showIcon={false} />,
    },
    {
      key: "crawl",
      ...
      render: (list) => (
        <Badge
          label={list.noCrawl ? IndexingAuditContent.crawl.off : IndexingAuditContent.crawl.on}
          tone={list.noCrawl ? "danger" : "success"}
        />
      ),
    },
    {
      ...
      filterValue: (list) =>
        list.hasUniquePermissions ? IndexingAuditContent.unique : IndexingAuditContent.inherited,
      render: (list) => (
        <Badge
          label={list.hasUniquePermissions ? IndexingAuditContent.unique : IndexingAuditContent.inherited}
    ...
    },
    {
      ...
      minWidth: 110,
      sortValue: (list) => list.indexedCount ?? -1,
      render: (list) => <span>{list.indexedCount === undefined ? "-" : formatNumber(list.indexedCount)}</span>,
    },
    {
      key: "coverage",
      header: IndexingAuditContent.columns.coverage,
      minWidth: 130,
      sortValue: (list) => coveragePercent(list) ?? -1,
      filterValue: (list) => coverageBand(list, target),
      render: (list) => {
        const percent = coveragePercent(list);
        if (percent === undefined) return <span>-</span>;

        return (
          <Badge
            label={`${percent}%`}
            tone={list.noCrawl ? "neutral" : percent >= target ? "success" : percent === 0 ? "danger" : "warning"}
          />
        );
      },
    },
    {
      ...
      render: (list) => <span>{formatDate(list.lastItemModified)}</span>,
    },
    {
      key: "actions",
      ...
      render: (list) => {
        ...
        const shape = { id: list.listId } as Parameters<typeof api.advancedSettingsUrl>[0];

        return (
          <div style={{ display: "flex", gap: 2 }}>
            <IconButton
              iconName="Search"
              ariaLabel={`${IndexingAuditContent.actions.searchSettings}: ${list.title}`}
              tooltip={IndexingAuditContent.actions.searchSettings}
              onClick={() => window.open(api.advancedSettingsUrl(shape), "_blank", "noopener")}
            />
            <IconButton
              iconName="Permissions"
              ariaLabel={`${IndexingAuditContent.actions.permissions}: ${list.title}`}
              tooltip={IndexingAuditContent.actions.permissions}
              onClick={() => window.open(api.permissionsUrl(shape), "_blank", "noopener")}
            />
            <IconButton
              iconName="Settings"
              ariaLabel={`${IndexingAuditContent.actions.settings}: ${list.title}`}
              tooltip={IndexingAuditContent.actions.settings}
              onClick={() => window.open(api.settingsUrl(shape), "_blank", "noopener")}
            />
          </div>
        );
      },
...
export function coverageBand(list: ListIndexCoverage, target: number): string {
  if (list.noCrawl) return IndexingAuditContent.crawl.off;
  const percent = coveragePercent(list);
  if (percent === undefined) return "Not checked";
  if (percent === 0) return "Nothing indexed";
  return percent >= target ? "Meets target" : "Below target";
}

export const itemColumns: TableColumn<ItemIndexCheck>[] = [
  {
    key: "title",
  ...
  },
  {
    ...
    render: (item) => (
      <Badge
        ...
        tone={!item.indexed ? "danger" : item.stale ? "warning" : "success"}
      />
    ),
  },
  {
    ...
    render: (item) => <span>{formatDate(item.itemModified)}</span>,
  },
  {
    key: "indexedModified",
    header: IndexingAuditContent.columns.indexedModified,
    minWidth: 160,
    sortValue: (item) => item.indexedModified ?? "",
    render: (item) => <span>{item.indexedModified ? formatDate(item.indexedModified) : "-"}</span>,
  },
];

export function stateLabel(item: ItemIndexCheck): string {
  if (!item.indexed) return IndexingAuditContent.state.missing;
