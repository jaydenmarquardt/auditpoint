...
export interface UsersAuditView {
  totals: UsersTotals;
  addedByMonth: { label: string; value: number }[];
  activeByMonth: { label: string; value: number }[];
  usersByKind: { label: string; value: number }[];
  membersByGroup: { label: string; value: number }[];
  byDepartment: { label: string; value: number }[];
  profileCompleteness: { label: string; value: number }[];
  ungrouped: SiteUser[];
}
