...
export interface WebPartAuditData {
  ...
  rawPages: (CanvasPage & { siteUrl: string })[];
}
...
export interface WebPartTypeSummary {
  ...
  sharedValues: { key: string; value: string }[];
  iconName: string;
  iconUrl: string;
  description: string;
  group: string;
...
export interface WebPartAuditView {
  ...
  catalogueOnly: { id: string; title: string; group: string; iconName: string }[];
  topTypes: { label: string; value: number }[];
  busiestPages: WebPartPageSummary[];
  layoutSplit: { key: string; label: string; value: number }[];
}
