...
export const ActionsTab: React.FC<{ view: LinkAuditView }> = ({ view }) => {
  ...
  return (
    <Card title={LinkAuditContent.actions.title} subtitle={LinkAuditContent.actions.description}>
      <div
        style={{
          display: "grid",
      ...
      >
        {actions.map((action) => (
          ...
          >
            <p style={{ margin: 0, color: Theme.palette().textMuted }}>{action.description}</p>
            <div>
              <Button
                label={action.label}
                ...
                title={action.disabled ? LinkAuditContent.actions.disabled : action.description}
                onClick={action.run}
              />
            </div>
          </div>
